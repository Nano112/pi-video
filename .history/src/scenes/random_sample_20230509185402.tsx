...
export default makeScene2D(function* (view) {
	...
	yield* all(
		zeroLabelPosition(() => new Vector2(-50, 0), 1, easeInOutCubic),
		xOneLabelPosition(() => new Vector2(endX() + 20, 0), 1, easeInOutCubic),
		angleY(-Math.PI / 2, 2, easeInOutCubic),
		position(new Vector2(-xWidth() / 2, yHeight() / 2), 2, easeInOutCubic),
		scale(0.9, 2, easeInOutCubic)
	);
	...
	const redHexStr = RED.toString().slice(1);
	...
	group().add(
		new Node({
			...
			children: [
				...
				new Latex({
					// fill: RED,
					// define the color in the tex to use RED
					tex: () =>
						"\\definecolor{c}{HTML}{" + redHexStr + "} \\color{c} \\pi",
					position: () =>
						new Vector2(
							(-graphWidth / 2) * graphScale() - 60 * scale() * graphScale(),
							-Math.PI * graphScale()
