...
export default makeScene2D(function* (view) {
	...
	const steps = [
		...
		{
			color: "#00f",
			text: "Divide the number of points inside the circle by the total number of points",
		},
	];
	...
	// 	>
	// 		<Txt
	// 			position={() => rect1().getOriginDelta(Origin.Middle)}
	// 			fontSize={80}
	...
	steps.forEach((step, i) => {
		const stepRef = stepRefs[i];
		const stepValue = new Rect({
			fill: step.color,
			radius: layoutRadius,
			width: width,
			height: width,
		...
		stepRef().add(
			<Txt
				position={() => stepRef().getOriginDelta(Origin.Middle)}
				fontSize={80}
				fontFamily={"monospace"}
				fill={"#fff"}
				alignSelf={"center"}
