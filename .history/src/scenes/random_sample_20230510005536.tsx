...
export default makeScene2D(function* (view) {
	...
	const randomPointTop = createSignal(1.2);
				...
				<Txt
					...
					position={() =>
						new Vector2(randomXPos(), randomYPos()).add(
							new Vector2(0, 100 * scale() * randomPointTop())
						)
					}
					zIndex={100}
				/>
	...
	yield* all(
		zeroLabelPosition(() => new Vector2(-50, 0), 1, easeInOutQuint),
		xOneLabelPosition(() => new Vector2(endX() + 20, 0), 1, easeInOutQuint),
		angleY(-Math.PI / 2, 2, easeInOutQuint),
		position(new Vector2(-xWidth() / 2, yHeight() / 2), 2, easeInOutQuint),
		scale(0.9, 2, easeInOutQuint),
		randomPointTop(-1, 2, easeInOutQuint)
	);
	...
	group().add(
		new Latex({
			tex: () =>
				"\\color{" +
				ORANGE +
				"} \\pi\\color{white} \\approx 4 \\cdot \\frac{\\color{" +
				GREEN +
				"}" +
				...
				RED +
				"}" +
				(totalPoints() - pointsInsideQuandrant()) +
				"} = " +
				...
				new Line({
					...
					points: () => {
						...
						return points.map(
							(p) =>
								new Vector2(
									(p.x / sampleIterations) * graphScale() * graphWidth -
	...
	// yield* all(randomSampleRadius(0, 0.5, easeInOutCubic));
	...
	yield* all(
		angleY(0, 1, easeInOutCubic),
		position(new Vector2(-xWidth() / 2, 0), 1, easeInOutCubic)
		// scale(0.4, 1, easeInOutCubic)
	);
	...
	yield* all(
		...
		graphScale(0, 1, easeOutQuint),
		graphPosition(new Vector2(0, 0), 1, easeOutQuint),
		latexPosition(new Vector2(0, 0), 1, easeOutQuint),
		position(new Vector2(0, 0), 1, easeOutQuint)
	);
